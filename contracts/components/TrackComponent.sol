...
uint256 constant ID = uint256(keccak256("game.racing.trackcomponent.v1"));

struct Layout {
    uint256[] lineEntities;
    uint256[] checkpointEntities;
}

library TrackComponentStorage {
 ...
 * @dev track Component
 */
contract TrackComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param lineEntities line entities
     */
    function setValue(
        uint256 entity,
        uint256[] memory lineEntities,
        uint256[] memory checkpointEntities
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(lineEntities, checkpointEntities));
    }
    ...
    function getLayoutValue(
        ...
        value = TrackComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return lineEntities line entities
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256[] memory lineEntities, uint256[] memory checkpointEntities) {
        if (has(entity)) {
            Layout memory s = TrackComponentStorage.layout().entityIdToStruct[
                entity
            ];
            (lineEntities, checkpointEntities) = abi.decode(_getEncodedValues(s), (uint256[], uint256[]));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TrackComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.lineEntities);
        values[1] = abi.encode(s.checkpointEntities);
    }
    ...
    function setBytes(
        ...
        (s.lineEntities, s.checkpointEntities) = abi.decode(value, (uint256[], uint256[]));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.lineEntities, s.checkpointEntities) = abi.decode(values[i], (uint256[], uint256[]));
    ...
    function remove(
        ...
        delete TrackComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TrackComponentStorage.layout().entityIdToStruct[entities[i]];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.lineEntities = value.lineEntities;
        s.checkpointEntities = value.checkpointEntities;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.lineEntities, value.checkpointEntities));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.lineEntities, value.checkpointEntities);
    }
}
