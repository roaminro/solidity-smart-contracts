...
uint256 constant ID = uint256(keccak256("game.racing.turncomponent.v1"));

struct Layout {
    uint32 turn;
}

library TurnComponentStorage {
 ...
 * @dev Turn Component
 */
contract TurnComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = TurnComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (uint32 turn) {
        if (has(entity)) {
            Layout memory s = TurnComponentStorage.layout().entityIdToStruct[
                entity
            ];
            (turn) = abi.decode(_getEncodedValues(s), (uint32));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TurnComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.turn);
    }
    ...
    function setBytes(
        ...
        (s.turn) = abi.decode(value, (uint32));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.turn) = abi.decode(values[i], (uint32));
    ...
    function remove(
        ...
        delete TurnComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TurnComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.turn));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.turn);
    }
}
